...
/** Handlesss comment mutations that originate from REST (e.g. like/unlike). */
@Controller("comments")
export class CommentsController {
  ...
  async likeComment(
    ...
    @Body()
    body: { commentAuthorId: string; songId: string; stationId: string },
    ...
    const likePayload: WsCommentLikesUpdatedPayload = {
      commentId,
      action: "like",
      userId: likerId,
    };
    this.socketState.emitToStation(
      body.stationId,
  ...
  async unlikeComment(
    ...
    const unlikePayload: WsCommentLikesUpdatedPayload = {
      commentId,
      action: "unlike",
      userId: likerId,
    };
